...
class VarAllocator(private val options: CommandLineOptions) {
    ...
    private fun formatVars(program: Program, table: SymbolTable) {
        program.definitions.stream().filter { dec: GlobalDefinition? -> dec is ProcedureDefinition }
            .map { dec: GlobalDefinition -> dec as ProcedureDefinition }
            .forEach { procDec: ProcedureDefinition ->
                val entry = table.lookup(procDec.name) as ProcedureEntry
                formatVarsProc(procDec, entry)
            }
    }

    private fun formatVarsProc(procDec: ProcedureDefinition, entry: ProcedureEntry) {
        ...
        val zipped = IntRange(0, procDec.parameters.size - 1)
            ...
            .sortedWith(
                Comparator.comparing { p: Pair<ParameterDefinition, Pair<VariableEntry?, ParameterType>> ->
        ...
        if (procDec.variables.isNotEmpty()) {
            ...
            // process VariableDefinitions by building a sorted set of pairs of offset x name
